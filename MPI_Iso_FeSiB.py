...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov  3 16:39:07 2025

@author: ag0406

@author: ag0406

Produce simulations of Isothermal heat treatments to investigate Rm of distributions 

Only minor adjustments should be needed to change phases or system

For the substitutional solution model

Transfering the model of second order transition and generally updated one. 03/11-25
...
def MPI_Nuc_FeSiB_Iso(x0,T):
    ...
    while ftot<fmax:
        ...
        if T < Tg or it > 1e6 and ftot < 1e-5 or it > 5e6 and ftot < 1e-2 or it > 10e6 or Time > 1e2:
            No_dc_flag = True
            break
    ...
    if not No_dc_flag:
        ...
        print('-------------------------')
        print('Failed simulation, too slow.')
        simtime=str(round((time.time()-simt)/60,1))
        print(f"wall-time:{simtime} min\n")
        print('-------------------------')
