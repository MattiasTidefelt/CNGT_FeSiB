...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov  5 11:22:18 2024

@author: ag0406
"""

def LiqPhase_dT(T,xFe,xSi,xB,PD):
    import numpy as np
    from scipy.optimize import minimize_scalar
    
    if PD == 'Liquid':
        from LiqFeSiB_dT import Gibbs_Liq_dT 
        from LiqFeSiB_dT import dTransE
    
        [G_Liq_B_0,G_Liq_Fe_0,G_Liq_Si_0]=Gibbs_Liq_dT[0]
        [G_Liq_FeB_0,G_Liq_FeB_1,G_Liq_FeB_2]=Gibbs_Liq_dT[1]
        [G_Liq_FeSi_0,G_Liq_FeSi_1,G_Liq_FeSi_2,G_Liq_FeSi_3]=Gibbs_Liq_dT[2]
        [G_Liq_SiB_0,G_Liq_SiB_1,G_Liq_SiB_2]=Gibbs_Liq_dT[3]
        [G_Liq_FeSiB_0,G_Liq_FeSiB_1,G_Liq_FeSiB_2]=Gibbs_Liq_dT[4]
    else:
        from SolidSolFeSiB_dT import Gibbs_SolidSol_dT 
        from SolidSolFeSiB_dT import dTransGMO
        
        [G_Liq_B_0,G_Liq_Fe_0,G_Liq_Si_0]=Gibbs_SolidSol_dT[0]
        [G_Liq_FeB_0,G_Liq_FeB_1,G_Liq_FeB_2]=Gibbs_SolidSol_dT[1]
        [G_Liq_FeSi_0,G_Liq_FeSi_1,G_Liq_FeSi_2,G_Liq_FeSi_3]=Gibbs_SolidSol_dT[2]
        [G_Liq_SiB_0,G_Liq_SiB_1,G_Liq_SiB_2]=Gibbs_SolidSol_dT[3]
        [G_Liq_FeSiB_0,G_Liq_FeSiB_1,G_Liq_FeSiB_2]=Gibbs_SolidSol_dT[4]
    ...
    Gide=R*(xB*np.log(xB) + xSi*np.log(xSi) + xFe*np.log(xFe))
    ...
    eta= 0
    if PD == 'Liquid':
        dGtrans = dTransE(T,xFe,xSi,xB)
        dGm=Gref+Gide+Gex + dGtrans
    else:   
        dGm=(Gref+Gide+Gex + dTransGMO(T,xFe,xSi,xB,PD))
        if PD == "bcc":
            L1 = -2*1260*R
            dGm, eta, _, _, _, _ = minimize_eta_and_get_sites(dGm ,xFe, xSi, xB, T, L1)
