def Gibbs_Func(T,xFe,xSi,xB,PD):
    import numpy as np
    ...
    ######################
    if PD == 'Liquid':
        [Gtrans,_] = TransE(T,xFe,xSi,xB)
        Gm=Gref+Gide+Gex + Gtrans
    else:   
        Gm=(Gref+Gide+Gex + TransGMO(T,xFe,xSi,xB,PD))
        if PD == "bcc":
            L1 = -2*1260*R
            Gm, eta, _, _, _, _ = minimize_eta_and_get_sites(Gm ,xFe, xSi, xB, T, L1)
