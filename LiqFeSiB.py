...
def TransE(T,xFe,xSi,xB):    
    ...
    if np.isscalar(xFe):
        if xFe > 0 and xSi > 0 and xB > 0:
            ...
            # Gtrans = GMO(Tk,T,abs(alpha),p)
        else:
            Gtrans = 0
    elif len(xFe) > 0:
        if xFe.any() > 0 and xSi.any() > 0 and xB.any() > 0:
            Gtrans = GMO(Tk/3,T,abs(alpha)/3,p)
            # Gtrans = GMO(Tk,T,abs(alpha),p)
        else:
            Gtrans = 0
...
def LiqFeSiB(T,PD):
    ...
    # CONSTITUENT LIQUID:L :B,FE,SI :  !
        ...
        #BFeSi     (L0)
        G_BFeSi_0= np.array([-5.23427598E-06, 0, 0, 0, 0, 0, 0, 0])
        G_BFeSi_1= np.array([-55686, 0, 0, 0, 0, 0, 0, 0])
        G_BFeSi_2= np.array([93217, 0, 0, 0, 0, 0, 0, 0])
    ...
    Gibbs_Liq=[Ref,FeB,FeSi,BSi,Ternary]
