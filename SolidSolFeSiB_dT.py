...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug 26 17:29:59 2025
...
def dTransGMO(T,xFe,xSi,xB,PD):   
    
    if PD=='bcc': # 2 1   3 !
        TC_Fe= 1043
        ETCFESI = 63
        
        TC = TC_Fe*xFe + xFe*xSi*8*ETCFESI*(xFe-xSi)  
        BMAGN= 2.22*xFe
        p = 0.4
        ...
        TC= 201/3 # -201 TN (antiferromagnetic Neel Temperature) 
        BMAGN= 2.1/3*xFe#-2.1
        p = 0.28
        # dGmo = GMO(TC,T,BMAGN,p)
        dGmo=0
...
def SolidSolFeSiB_dT(T,PD):

    #GHSER data
    global Gibbs_SolidSol_dT
    #------------ 
    ...
    # ONSTITUENT BCC_A2  :B,FE%,SI : VA% :  !    
        ...
        L1 = - 11544
        L2 = 3890
        L0BCC = np.dot(np.array([-27809, 11.62, 0, 0, 0, 0, 0, 0]),PoldT)[0]
        #------------ B
        if T<=6000:
        ...
        #FeBSi    (L0)
        G_FeBSi_0= np.array([0, 0, 0, 0, 0, 0, 0, 0])
        G_FeBSi_1= np.array([0, 0, 0, 0, 0, 0, 0, 0])
        G_FeBSi_2= np.array([0, 0, 0, 0, 0, 0, 0, 0])
    ...
    Gibbs_SolidSol_dT=[Ref,FeB,FeSi,BSi,Ternary]
