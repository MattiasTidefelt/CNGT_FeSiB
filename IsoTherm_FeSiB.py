...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov  3 16:06:13 2025
...
Starting the attempt of implementing Fe-Si-B multiphase model 03/11-25

Aims to perform isothermal crystallisation simulations, to target rm rather than f
"""
# Modules
...
basedir = "/Users/ag0406/Documents/Mattias/PhD/CNT/FeBSi/Iso_Simulation/"
...
# x0 = np.array([0.767 , 0.178, 0.055])

if T > 1043*x0[0] + x0[0]*x0[1]*8*63*(x0[0]-x0[1]):     # Tc according to assessment
    isHeating = False
    isQuenching = True
else:
    isHeating = True
...
Hf_Sf, PD = CheckDcAtMatrixComp(PD,Hf_Sf,x0)            # Get reduced PD if phase does not exhibit dc in temp itnerval, but at matrix composition     
...
OutDir =f'{basedir}/Comp_{x0Str}/Iso_{T}_K/'
...
while f[-1]<fmax:
    ...
    if np.linalg.norm(xm-xm_old)>1e-3:            # Only update things if something has changed, i.e., T
    
        LiqFeSiB(T,'Liquid')      #Update Liquid Gibbs(T) Polynomials
        TangCoeff = Gibbs_Tangent(T,xm[0],xm[1],xm[2],'Liquid')
        fp = Gibbs_Func(T,xm[0],xm[1],xm[2],'Liquid') #Gibbs(p)
        ...
        if dc[p] > dcMin:           # Avoid corruption by only allowing possitive driving force         
            ...
            if p != SolInActiveIdx:
                ...
                if Tau <0:
                    Tau = 0
                ##################       
                Jdis = np.sum(N[p,np.where(r[p,:]<0.4*r_star[p])])
                # dN[p] = (1-f[p])*J_st*dt
                ...
                N[p,np.where(r[p,:]<0.5*r_star[p])] = 0
...
if No_dc_flag:
    print(f"No driving force at {T} K")    
else:    
    Final_status = (
        f"\r t:{Time:2.4f}, "
        f"T:{T:4.1f}, "
...
#%%
    ...
    FigpathOut=f'{OutDir}Velocities_ISO.png'
    ...
    FigpathOut=f'{OutDir}PSD_ISO.png'
    fig2.savefig(FigpathOut, bbox_inches = "tight")
    FigpathOut=f'{OutDir}GrowthPar_ISO.png'
